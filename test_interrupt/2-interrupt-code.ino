...
volatile uint8_t port_d_history = 0xFF; 
...
int main(void)
{
    ...
    PCMSK0 |= ((1 << PCINT0) | (1 << PCINT1));   // enable PCINT from PB pins 
    PCMSK1 |= (1 << PCINT8) | (1 << PCINT9);   // enable PCINT from PC pins  
    PCMSK2 |= (1 << PCINT20) | (1 << PCINT23); // enable PCINT from PD pins 
...
ISR (PCINT1_vect)
{
    ...
    if(changed_bits & (1 << PINC0)) //pin A raised an interrupt
    {
...
{
    uint8_t changed_bits;
    changed_bits = PIND ^ port_d_history;
    port_d_history = PIND;

    if(changed_bits & (1 << PIND4)) //pin A raised an interrupt
    {
